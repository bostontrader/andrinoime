...
static void latinime_BinaryDictionary_getHeaderInfo(JNIEnv *env, jclass clazz, jlong dict,
    ...
    const DictionaryHeaderStructurePolicy *const headerPolicy =
            dictionary->getDictionaryStructurePolicy()->getHeaderStructurePolicy();star/
    //JniDataUtils::putIntToArray(env, outHeaderSize, 0 /star index star/, headerPolicy->getSize());
    //JniDataUtils::putIntToArray(env, outFormatVersion, 0 /star index star/,
    ...
    //for (DictionaryHeaderStructurePolicy::AttributeMap::const_iterator it = attributeMap->begin();
        ...
        //JniDataUtils::outputCodePoints(env, keyCodePointArray, 0 /star start star/,
                //it->first.size(), it->first.data(), it->first.size(),
                //false /star needsNullTermination star/);
        ...
        //JniDataUtils::outputCodePoints(env, valueCodePointArray, 0 /star start star/,
...
/starstatic int latinime_BinaryDictionary_getFormatVersion(JNIEnv *env, jclass clazz, jlong dict) {
    Dictionary *dictionary = reinterpret_cast<Dictionary *>(dict);
...
static void latinime_BinaryDictionary_getSuggestions(JNIEnv *env, jclass clazz, jlong dict,
    ...
    Dictionary *dictionary = reinterpret_cast<Dictionary *>(dict);
    // Assign 0 to outSuggestionCount here in case of returning earlier in this method.star/
    //JniDataUtils::putIntToArray(env, outSuggestionCount, 0 /star index star/, 0);
    //if (!dictionary) {
        /starreturn;
    }
    ...
    // Output valuesstar/
    /star By the way, let's check the output array length here to make sure star/
    ...
    /starASSERT(outputAutoCommitFirstWordConfidenceLength == 1);
    ...
    env->GetFloatArrayRegion(inOutLanguageWeight, 0, 1 /star len star/, &languageWeight);
...
static jint latinime_BinaryDictionary_getNextWord(JNIEnv *env, jclass clazz,
    ...
    const int nextToken = dictionary->getNextWordAndNextToken(token, wordCodePoints,
            &wordCodePointCount);
    JniDataUtils::outputCodePoints(env, outCodePoints, 0 /star start star/,
            MAX_WORD_LENGTH /star maxLength star/, wordCodePoints, wordCodePointCount,
            false /star needsNullTermination star/);
    ...
    JniDataUtils::putBooleanToArray(env, outIsBeginningOfSentence, 0 /star index star/,
...
static bool latinime_BinaryDictionary_addUnigramEntry(JNIEnv *env, jclass clazz, jlong dict,
    ...
    const UnigramProperty unigramProperty(isBeginningOfSentence, isNotAWord,
            isBlacklisted, probability, timestamp, 0 /star level star/, 1 /star count star/, &shortcuts);
...
static bool latinime_BinaryDictionary_addNgramEntry(JNIEnv *env, jclass clazz, jlong dict,
    ...
    const BigramProperty bigramProperty(&bigramTargetCodePoints, probability,
            timestamp, 0 /star level star/, 1 /star count star/);
...
static int latinime_BinaryDictionary_addMultipleDictionaryEntries(JNIEnv *env, jclass clazz,
    ...
    for (int i = startIndex; i < languageModelParamCount; ++i) {
        ...
        // Use 1 for count to indicate the word has inputted.
        const UnigramProperty unigramProperty(false /star isBeginningOfSentence star/, isNotAWord,
                isBlacklisted, unigramProbability, timestamp, 0 /star level star/, 1 /star count star/,
                &shortcuts);
        ...
        if (word0) {
            ...
            const PrevWordsInfo prevWordsInfo(word0CodePoints, word0Length,
                    false /star isBeginningOfSentence star/);
            dictionary->addNgramEntry(&prevWordsInfo, &bigramProperty);
        }
        if (dictionary->needsToRunGC(true /star mindsBlockByGC star/)) {
            return i + 1;
        }
...
static DictionaryStructureWithBufferPolicy::StructurePolicyPtr runGCAndGetNewStructurePolicy(
    ...
    return DictionaryStructureWithBufferPolicyFactory::newPolicyForExistingDictFile(
            dictFilePath, 0 /star offset star/, 0 /star size star/, true /star isUpdatable star/);
}
...
static bool latinime_BinaryDictionary_migrateNative(JNIEnv *env, jclass clazz, jlong dict,
    ...
    do {
        ...
        if (dictionaryStructureWithBufferPolicy->needsToRunGC(true /star mindsBlockByGC star/)) {
