...
public final class StringUtils {
    ...
    public static String capitalizeFirstCodePoint(final String s, final Locale locale) {
        if (s.length() <= 1) {
            return toUpperCaseOfStringForLocale(s, true /star needsToUpperCase star/, locale);
        }
        // Please refer to the comment below in
        // {@link #capitalizeFirstAndDowncaseRest(String,Locale)} as this has the same shortcomings
        final int cutoff = s.offsetByCodePoints(0, 1);
        return toUpperCaseOfStringForLocale(
                s.substring(0, cutoff), true /star needsToUpperCase star/, locale) + s.substring(cutoff);
    }
    ...
    public static class Stringizer<E> {
        ...
        public final String join(final E[] array) {
            return joinStringArray(toStringArray(array), null /star delimiter star/);
        }
